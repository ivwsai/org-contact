 ...
 * @param deep,整形数字，默认值：0，用于设置初始的深度，如设置为0，则第一个select的name属性就是sel0,第二个就是sel1,接着sel2,sel3,sel4以此排下去。
 * @return 调用它的对象
 * @type jQuery对象
 *
 */
;(function($) {
	$.fn.json2select=function(json,dft,name,deep) {
		...
		if (json[0]) {
			//新建一个select
			var slct=$("<select name='"+name+$("select",_this).length+"' class=\"selectpicker\"></select>");
	...
	$.fn.areaSelect=function(dft,name,deep) {
		var _this=this;
		$.get(URL + "static/js/area.json", function(data){
			$(_this).json2select(data,dft,name,deep);
		}, 'json');
	}
})(jQuery);