...
define(function (require, exports, module) {
    ...
    var Util = {
        ...
        getCateImgUrl: function(cid, size) {
            return Global.base_url + 'api/jackpot/img'+
                '?cid='+cid+
        ...
        getPrizeImgUrl: function(pid, size) {
            return Global.base_url + 'api/prize/img'+
                '?pid='+pid+
                '&unitid='+Global.unitid+
                '&size='+size+
                '&timestamp='+new Date().getTime();
        ...
        getCurrentPage: function(pagination, pageSize, page) {
            if(Util.isNum(page)) {
                return page;
            } else if(page === 'current') {// 跳转到当前页
                return pagination.get('current');
            } else if(page === 'last') {// 跳转到最后一页
                var total = pagination.get('total');
                var currentPage = Math.ceil(+total/pageSize);
                return total % pageSize === 0 ? ++currentPage : currentPage;// 数据已满页，+1
            } else if(page === 'remove') {// 删除后跳转到当前页
                var total = pagination.get('total');
                var currentPage = pagination.get('current');
                return total % pageSize === 1 ? --currentPage : currentPage;// 只剩一条数据,-1
            } else if(page === 'removeAll') {// 删除所有后跳转到当前页
                var currentPage = pagination.get('current');
