...
define(function(require, exports, module) {
    ...
    var ImportXsl = {
        // 初始化各变量
        SUCCESS_NUM: 0, // 导入成功的条数
        FAIL_NUM   : 0, // 导入失败的条数
        START_INDEX: 2,  // 第一条导入记录的索引
        CURR_INDEX : 2, // 当前正在导入的记录索引
        LIST_DATA  : null, // 缓存flash返回的数据
        ERROR_DATA : [null, null],    // 导入出错的数据

        _CALLBACK  : null, // 提交的数据
        _URL       : '',    // 提交的地址
        _ACTIVE    : true,
        ...
        __init: function(url, callback, index) {
            ...
            window.onbeforeunload = function(e) {
                if ($('#process').html() != '') {
                    ...
                    if (e) {
                        var targetHtml = $(window.document.activeElement).html(); //获取当前活动链接
