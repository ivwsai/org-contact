...
/**
 *
 * HTML帮助类，提供对HTML操作的通用方法包括HTML安全和标签等
 * @package Netap
 * @category System
 * @author OAM Team
 *
...
class Netap_HTML
{
    ...
    {
        return htmlspecialchars((string)$value, ENT_QUOTES, Netap::$charset, $double_encode);
    }
    ...
    {
        return htmlentities((string)$value, ENT_QUOTES, Netap::$charset, $double_encode);
    }
    ...
    public static function anchor($uri, $title = NULL, array $attributes = NULL, $protocol = NULL)
    {
        ...
        if ($uri === '') {
            ...
            $uri = Netap_URL::base(FALSE, $protocol);
        } else {
            if (strpos($uri, '://') !== FALSE) {
                if (Netap_HTML::$windowed_urls === TRUE and empty($attributes['target'])) {
                    // Make the link open in a new window
                    $attributes['target'] = '_blank';
                }
            } elseif ($uri[0] !== '#') {
                // Make the URI absolute for non-id anchors
                $uri = Netap_URL::site($uri, $protocol);
            }
        }
        ...
        return '<a' . Netap_HTML::attributes($attributes) . '>' . $title . '</a>';
    }
    ...
    public static function file_anchor($file, $title = NULL, array $attributes = NULL, $protocol = NULL)
    {
        ...
        $attributes['href'] = Netap_URL::base(FALSE, $protocol) . $file;
    ...
    public static function email($email)
    {
        ...
        return str_replace('@', '&#64;', Netap_HTML::obfuscate($email));
    }
    ...
    public static function mailto($email, $title = NULL, array $attributes = NULL)
    {
        ...
        return '<a href="&#109;&#097;&#105;&#108;&#116;&#111;&#058;' . $email . '"' . Netap_HTML::attributes($attributes) . '>' . $title . '</a>';
    }
    ...
    public static function style($file, array $attributes = NULL, $index = FALSE)
    {
        if (strpos($file, '://') === FALSE) {
            // Add the base URL
            $file = Netap_URL::base($index) . $file;
        }
        ...
        return '<link' . Netap_HTML::attributes($attributes) . ' />';
    }
    ...
    public static function script($file, array $attributes = NULL, $index = FALSE)
    {
        ...
        return '<script' . Netap_HTML::attributes($attributes) . '></script>';
    }
    ...
    public static function image($file, array $attributes = NULL, $index = FALSE)
    {
        ...
        return '<img' . Netap_HTML::attributes($attributes) . ' />';
    }
    ...
    public static function attributes(array $attributes = NULL)
    {
        ...
        $sorted = array();
        foreach (Netap_HTML::$attribute_order as $key) {
        ...
        foreach ($attributes as $key => $value) {
            ...
            $compiled .= ' ' . $key . '="' . Netap_HTML::chars($value) . '"';
        }
